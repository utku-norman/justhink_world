...
class EnvironmentScene(object):
    ...
    def _update_attempt_label(self):
        ...
        if state.max_attempts is not None:
            self._attempt_label.text = 'Attempt: {}/{}'.format(
                state.attempt_no,
                state.max_attempts)
    ...
    def _update_submit_box(self):
        if self._state.is_submitting:
            self._confirm_rect.visible = True
            self._confirm_text_label.text = 'Do you want to submit?'
            self._yes_label.text = 'Ok'
            self._no_label.text = 'Cancel'
        else:
            self._confirm_rect.visible = False
            self._confirm_text_label.text = ''
            self._yes_label.text = ''
            self._no_label.text = ''
