...
class ObserverWindow(pyglet.window.Window):
    ...
    def __init__(self, world, caption='Observer Window', width=1920,
                 height=1080, offset=(1920, 0), screen_index=0):

        self.scene = ObserverScene(world, width=width, height=height)
...
class ObserverScene(Scene):
    ...
    def _init_graphics(self, width, height, max_level, batch=None):
        ...
        graphics.observes_heading_label = pyglet.text.Label(
            '(agent observes)', x=outer_pad[0], y=y, font_name='Sans',
...
class MentalScene(ObserverScene):
    """docstring for MentalWindow"""
